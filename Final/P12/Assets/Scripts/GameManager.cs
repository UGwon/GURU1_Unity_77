...
public class GameManager : MonoBehaviour
{
    ...
    public void GameStart()
    {
        ...
        */
        ...
        DataMgr datamgr = GameObject.Find("DataMgr").GetComponent<DataMgr>();
        Player player = GameObject.FindGameObjectWithTag("Player").GetComponent<Player>();

        if (datamgr.currentCharacter == Character.Usi)
        {
            GameManager.Instance.FeverTime();
        }
        else if (datamgr.currentCharacter == Character.Wendy)
        {
            player.AvoidChange = true;
        }
    }
    ...
    {
        if (ScoreManager.Instance.currentScore > 2000 && ScoreManager.Instance.currentScore < 4999)
        {
            background1.SetActive(false);
            background2.SetActive(true);
        }
        if (ScoreManager.Instance.currentScore > 5000 && ScoreManager.Instance.currentScore < 7999 )
        {
            background2.SetActive(false);
            background3.SetActive(true);
        }
        if (ScoreManager.Instance.currentScore > 8000)
        {
            background3.SetActive(false);
            background4.SetActive(true);
        }

        /*if (ScoreManager.Instance.currentScore > 11000 && ScoreManager.Instance.currentScore < 12999)
        {
            background4.SetActive(false);
            background5.SetActive(true);
        }

        if (ScoreManager.Instance.currentScore > 13000 && ScoreManager.Instance.currentScore < 14999)
        {
            background5.SetActive(false);
            background6.SetActive(true);
        }

        if (ScoreManager.Instance.currentScore > 15000)
        {
